...
public class REV_Module {
  ...
  public REV_Module(
    ...
    if (motor == Motors.NEO_VORTEX) {
      ...
      driveVortex.setInverted(
        MK4i.getDriveMotorReversed(location, side_reversed)
      );
      ...
      DriverStation.reportError(
        location.toString() + "Module motors provided are not REV Motors",
        true
      );
      moduleEnabled = false;
    }
    ...
    if (abs_encoder == AbsoluteEncoder.CAN_CODER) {
      ...
      DriverStation.reportError(
        location.toString() + "Error with provided Absolute Encoder",
        true
      );
      moduleEnabled = false;
    }
  ...
  public SwerveModuleState getModuleState() {
    return new SwerveModuleState(
      getDriveVelocity(),
      new Rotation2d(getPivotPosition())
    );
  }
